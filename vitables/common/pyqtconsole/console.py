...
from qtpy.QtGui import QFontMetrics, QTextCursor, QClipboard, QTextOption
...
class BaseConsole(QFrame):
    ...
    def _insert_output_text(self, text,
        ...
        if lf:
            self.process_input('')
        # record all output lines    
        for line in range(start_line, cursor.blockNumber()):
            self.output_lines.add(line)
        self.output_active = False
    ...
    def _get_input_valid(self, line_number):
        return not (line_number in self.output_lines or self.output_active)
...
class PythonConsole(BaseConsole):
    ...
    def __init__(self, parent=None, locals=None, formats=None):
        ...
        self.highlighter = PythonHighlighter(
            self.edit.document(), self._get_input_valid, formats=formats)
